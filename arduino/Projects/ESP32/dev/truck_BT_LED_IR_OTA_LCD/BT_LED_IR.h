...
void btTASK( void * pvParameters ){
  ...
  a2dp_sink.start(btDeviceName, false);
...
void ledTASK( void * pvParameters ){
  ...
  if(wifiMulti.run() == WL_CONNECTED) {
    ...
    Serial.println("Initializing Starfield");
    za = random(256);
  zb = random(256);
  zc = random(256);
  zx = random(256);
...
/*
 * More audio code. Meh, no, maybe.
 *
  // take some initial audio measurements to establish the "quiet" level
  for(int i=0; i<20; i++) {
    quietLevel += analogRead(audioSamplePin); // 0-1023
    delay(25);
  }
  */
//  quietLevel /= 20;
//  Serial.print("\nquietLevel is "); Serial.println(quietLevel);
  ...
  for(;;){
/*    
 *     This is code for audio smapling. Not a fan but the people (wife and kiddos
 *     seem to want it. Meh, we'll see.
 *     
 *     uint16_t audioSample = abs(analogRead(audioSamplePin) - quietLevel);
