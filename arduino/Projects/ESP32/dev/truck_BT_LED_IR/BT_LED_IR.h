...
void btTASK( void * pvParameters ){
  ...
  xSemaphoreGive(ota_beginning);
  vTaskDelete(bt_stream);
}
...
void ledTASK( void * pvParameters ){
  ...
  ArduinoOTA
    .onStart([]() {
      int ota_beginning = 0;
      if( xSemaphoreTake(xSemaphore, portMAX_DELAY)){
        Serial.print("OTA flag set to "); Serial.println(ota_beginning++);
        xSemaphoreGive(ota_beginning);
        }
      while (ota_beginning != 2) {
        delay(1000);
        if( xSemaphoreTake( xSemaphore, portMAX_DELAY)){
          break;
        }
  ...
  Serial.print(F("NEC Protocol initiated. IR ready at hardware pin "));
